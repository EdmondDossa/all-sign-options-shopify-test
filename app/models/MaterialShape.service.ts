...
export default class MaterialShapeService {
  static async getAll(sessionId: string, configurationId: number, materialId: number): Promise<ConfigShape[] | null> {
    try {
      let configuration: ConfigurationType = await ConfigurationService.getConfiguration(configurationId, sessionId);
      let shapes = configuration["data"]["materials"][materialId]["data"]["shapes"];
  ...
  static async add(
    ...
    try {
      ...
      if (materialData instanceof Object && "shapes" in materialData) {
        const shapes = configuration["data"]["materials"][materialId]["data"]["shapes"];
          configuration["data"]["materials"][materialId]["data"]["shapes"]=[...shapes||[], shape]
              
      } else {
        configuration["data"]["materials"][materialId]["data"] = { ...materialData||{},'shapes':[shape]}
      }
      ...
      if (configuration["data"]["materials"][materialId]["data"]["shapes"]) {
        ...
        return Promise.resolve(configuration["data"]["materials"][materialId]["data"]["shapes"]);
      }
      return Promise.resolve(null);
    } catch (error) {
  ...
  static async  update(
    ...
    try {
      let configuration: ConfigurationType = await ConfigurationService.getConfiguration(configurationId, sessionId);
      let shapes: ConfigShape[]|null = configuration["data"]["materials"][materialId]["data"]["shapes"];
      if (Array.isArray(shapes) && configuration["data"]["materials"][materialId]["data"]["shapes"][id]) {
        configuration["data"]["materials"][materialId]["data"]["shapes"][id] = shape;
        configuration =  await ConfigurationService.updateConfiguration(configuration,sessionId)
  ...
  static async  delete(
    ...
    try {
      ...
      let shapes: ConfigShape[]|null = configuration["data"]["materials"][materialId]["data"]["shapes"];
      if (Array.isArray(shapes) && configuration["data"]["materials"][materialId]["data"]["shapes"][id]) {
        configuration["data"]["materials"][materialId]["data"]["shapes"] = shapes.filter((curr,index)=> index!= id);
        configuration =  await ConfigurationService.updateConfiguration(configuration,sessionId)
        return Promise.resolve(shapes);
      }
      return Promise.resolve(null);
