...
export default class MaterialService {
  static async getAll(sessionId: string, configurationId: number): Promise<Material[] | null> {
    try {
  ...
  static async add(
    ...
    try {
      ...
      if (data instanceof Object && "materials" in data) {
        configuration["data"]["materials"]=[...configuration["data"]["materials"]||[],material]
      } else {
  ...
  static async update(
    ...
    try {
      let configuration: any = await ConfigurationService.getConfiguration(configurationId, sessionId);
      let materials: Material[] = configuration["data"]["materials"]??[];
      if (Array.isArray(materials) && materials[id]["type"] == material["type"]) {
  ...
  static async delete(
    ...
    try {
      ...
      if (Array.isArray(materials)) {
        configuration["data"]["materials"] = materials.filter((curr,index)=> index!= id);
        configuration =  await ConfigurationService.updateConfiguration(configuration,sessionId)
        return Promise.resolve(materials);
      }
      return Promise.resolve(null);
