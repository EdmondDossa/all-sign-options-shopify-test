...
const HeaderTopMenu = () => {
  ...
  useEffect(() => {
    ...
    for (const match of matches) {
      if (match.params.configId) {
        ...
        if(match.data && (match.data as any)?.configuration?.name != undefined){
          // console.log(match.data.configuration?.name, "655624")
          newConfigName = (match.data as any)?.configuration?.name
        }
      }

      if (match.id.includes("configId.materials")) {
      ...
      if (match.id.includes("configId.addditonal-options")) {
    ...
    setConfigName(newConfigName === configName ? configName : newConfigName)
    setCurrentTopMenu(newCurrentTopMenu === currentTopMenu ? currentTopMenu : newCurrentTopMenu);
  ...
  return (
    <div>
      <div className="topbar-aso" >
        {
          configId && <>
            <div onClick={()=> navigate('/app/configuration')} style={{
              display: 'flex',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'center' }} >
                  ...
                  {currentTopMenu != "addditonal-options" && <button onClick={() => navigateToMenu("addditonal-options", configId)} className="aso-additonal-btn" >
