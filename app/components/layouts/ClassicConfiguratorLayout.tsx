...
export default function ClassicConfiguratorLayout() {
  ...
  const handlePreview = () => {
    ...
    navigate(
      `/app/configuration/${previewConfigId}/preview?returnTo=${returnTo}`,
    );
  };
  ...
  const filteredMainGroups = useMemo(
    () => filteredGroups.filter((group) => group.title !== "Settings"),
    [filteredGroups],
  );

  const filteredSettingsGroup = useMemo(
    () => filteredGroups.find((group) => group.title === "Settings") || null,
    [filteredGroups],
  );
          ...
          <Card>
            ...
            >
                  ...
                  <Box key={group.title} padding="250">
                    ...
                    >
                      ...
                      >
                        <Icon
                          source={
                            groupIsExpanded ? ChevronDownIcon : ChevronRightIcon
                          }
                          tone="subdued"
                        />
                      </div>
              ...
              {filteredSettingsGroup ? (() => {
                const group = filteredSettingsGroup;
                const groupIsActive = activeGroupTitle === group.title;
                const groupIsExpanded =
                  searchMenu.trim().length > 0 || expandedGroup === group.title;
