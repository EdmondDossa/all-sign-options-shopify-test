...
interface Props {
  ...
  onChange?: (jacket: JacketType) => void;
  onSubmit: (jacket: JacketType) => void;
  onClose: () => void;
}
export interface JacketType {
  id?: number;
  label: string;
  description: string;
  type?: "white" | "colored";
...
export default function JacketForm({
  ...
  const defaultSize: JacketType | any = isEditing
    ? jacket
    : {
        id: null,
        label: "",
  ...
  const onInternalSubmit = (data: JacketType) => {
    try {
      onSubmit(data);
    } catch (e) {
        ...
        <InlineStack gap="300" align="space-between">
          <Box padding="300" background="bg-surface">
            <Text as="p" variant="headingMd">
              Jackets <NextLtrIcon width={6} height={10} />{" "}
              {isEditing ? "Edit Jacket" : "Add new Jacket"}
            </Text>
          </Box>
      ...
      <Form onSubmit={handleSubmit(onInternalSubmit)}>
        <Box paddingInline="300" paddingBlock="200" as="div">
          <SpacingBackground backgroundColor="#F8F9FB">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    helpText="The effect will apply to the visualiser when the sign light is turned off."
                    options={[
            ...
            <Box>
              <Controller
                name="colorsExclusions"
                control={control}
                render={({ field }) => (
                  <MultiCombobox
                    label={"Color Exclusions"}
                    placeholder="Select colors"
                    helpText="Select the colours that this jacket option will NOT be available for."
